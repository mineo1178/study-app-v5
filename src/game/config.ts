...
export const SPARKLE_STAGES = [{ battleId: "sparkle-stage-1", rivalId: "sparkle", name: "Sparkle Stage 1", requiredMembers: 3, requiredSongs: 2, requiredVenueId: "mini-live-house", stats: { vocal: 40, dance: 42, song: 40, character: 34 }, rewards: { winFans: 300, winPoints: 10, participationFans: 20, participationPoints: 2 } }] as const;
...
export const createInitialGameState = (): ProducerGameState => ({
  ...
  claimedSessionIds: [], lessonsCompleted: 0, songsCompleted: 0, rivalEventsCompleted: 0, producerStars: 0, boostRemainder: 0, milestones: {}, claimedRivalBattleIds: [], songs: SONGS.map((song, index) => ({ ...song, status: index === 0 ? "available" : "locked", level: 1, performanceCount: 0, songStats: { vocal: 0, harmony: 0, dance: 0, character: 0, lyrics: 0, composition: 0, choreography: 0 } })),
});
export const normalizeGameState = (saved: Partial<ProducerGameState>): ProducerGameState => {
  ...
  return { ...initial, ...saved, members, songs, milestones: { ...initial.milestones, ...(saved.milestones ?? {}) }, claimedRivalBattleIds: saved.claimedRivalBattleIds ?? [] };
};
