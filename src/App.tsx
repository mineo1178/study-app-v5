...
import { Clock, Play, Pause, Plus, Trash2, CheckCircle, Circle, Edit2, ChevronDown, Award, X, Zap, Layers, History, LayoutDashboard, TrendingUp, Calendar as CalendarIcon, PieChart as PieChartIcon, BarChart2, AlertTriangle, RefreshCw, CloudOff, Cloud, FileText, FlaskConical, LogIn, LogOut } from 'lucide-react';
...
import { getFirestore, collection, doc, setDoc, getDocs, updateDoc, deleteDoc, serverTimestamp, writeBatch, query } from 'firebase/firestore';
// ==========================================
...
const getTasksCol = (database) => collection(database, 'families', FAMILY_ID, 'tasks');
const getTestsCol = (database) => collection(database, 'families', FAMILY_ID, 'tests');
const getTaskDoc = (database, id) => doc(database, 'families', FAMILY_ID, 'tasks', id);
const getTestDoc = (database, id) => doc(database, 'families', FAMILY_ID, 'tests', id);
// Cache Keys
...
const StrictTimer = React.memo(({ task, updateLocalTask, syncTaskToCloud, onSaveRecord, pauseAllOtherTasks }) => {
    ...
    useEffect(() => {
        if (!isRunning)
            setLocalSeconds(task.currentDuration);
        else
            setLocalSeconds(getAccurateSeconds());
    ...
    return (<div className="flex flex-col gap-3 w-full relative">
       <div className="flex items-center justify-between bg-slate-50 rounded-2xl p-3 border border-slate-200 shadow-inner w-full">
          ...
          <div className="flex items-center gap-2 pr-2">
             {!isRunning ? (<button onClick={handlePlay} className="w-12 h-12 flex items-center justify-center rounded-full bg-blue-600 text-white shadow-lg active:scale-95 hover:bg-blue-700 transition-all">
                  <Play size={20} fill="currentColor" className="ml-1"/>
               </button>) : (<button onClick={handlePauseClick} className="w-12 h-12 flex items-center justify-center rounded-full bg-amber-500 text-white shadow-lg active:scale-95 hover:bg-amber-600 transition-all">
                  <Pause size={20} fill="currentColor"/>
               </button>)}
          </div>
       </div>

       {showAutoPauseAlert && (<div className="absolute inset-0 z-10 flex items-center justify-center bg-white/95 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-amber-200 animate-in fade-in zoom-in-95">
       ...
       <button onClick={handleStopAndSave} disabled={localSeconds === 0} className="w-full bg-slate-800 text-white font-bold py-3.5 rounded-2xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
...
const ConfirmModal = ({ isOpen, onClose, onConfirm, title, message }) => {
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in">
...
const CreateUnitOverlay = ({ isOpen, onClose, onCreate }) => {
    const [unitNumber, setUnitNumber] = useState('');
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 bg-slate-900/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95">
        ...
        <button onClick={() => { if (parseInt(unitNumber) > 0) {
        onCreate(parseInt(unitNumber));
        setUnitNumber('');
        onClose();
    } }} disabled={!unitNumber} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold py-4 rounded-xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
          <Zap size={20} fill="currentColor"/> カリキュラムを作成
        </button>
      </div>
    </div>);
};
const AddCustomTaskModal = ({ isOpen, onClose, onAdd, unit, subject }) => {
    ...
    return (<div className="fixed inset-0 bg-slate-900/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95">
        ...
        <div className="space-y-4">
          ...
          <button onClick={() => { if (title) {
        onAdd(title, category);
        setTitle('');
        setCategory('予習シリーズ');
        onClose();
    } }} disabled={!title} className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl shadow-lg mt-2">追加</button>
        </div>
      </div>
    </div>);
};
const TestResultModal = ({ isOpen, onClose, onSave, initialData }) => {
    ...
    return (<div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
        ...
        <div className="space-y-3">
          ...
          <div className="border-t border-slate-100 pt-3">
            ...
            <div className="grid grid-cols-4 gap-2">
              {['math', 'japanese', 'science', 'social'].map(subj => (<div key={subj}>
...
const TaskDetailModal = ({ task, onClose, updateLocalTask, syncTaskToCloud, onSaveRecord, onDelete, pauseAllOtherTasks }) => {
    ...
    return (<div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative bg-slate-50 w-full max-w-md h-[90vh] sm:h-[85vh] sm:rounded-3xl rounded-t-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom-10">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6 scroll-smooth pb-10">
        ...
        ].map(s => (<button key={s.id} onClick={() => {
                updateLocalTask(task.id, { status: s.id, lastUpdatedAt: Date.now() });
            }} className={`flex-1 py-2.5 rounded-xl font-bold text-xs flex flex-col items-center justify-center gap-1 border transition-all ${task.status === s.id ? s.active : 'bg-white text-slate-400 border-slate-200 hover:bg-slate-50'}`}>
          ...
          <div className="space-y-2">
            ...
            <textarea value={task.currentMemo} onChange={(e) => updateLocalTask(task.id, { currentMemo: e.target.value })} placeholder="ここにつまづいた、次はこうする..." className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm font-medium text-slate-700 focus:outline-none focus:border-blue-500 resize-none h-20 shadow-sm"/>
          </div>
          
          <div className="space-y-3 pt-2">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-1">過去の履歴 ({task.history.length})</label>
            {task.history.length === 0 ? (<div className="text-center py-6 text-slate-300 text-xs font-bold bg-white rounded-xl border border-dashed border-slate-200">記録なし</div>) : (<div className="space-y-2">
                {[...task.history].reverse().map((h, i) => (<div key={h.id} className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm flex gap-3">
...
const SubjectSection = ({ unit, subject, tasks, cycleStatus, setDetailTaskId, onAddCustomTask }) => {
    const [filter, setFilter] = useState('all');
    const [isAddModalOpen, setAddModalOpen] = useState(false);
    const subjTasks = tasks.filter((t) => t.unit === unit && t.subject === subject);
    if (subjTasks.length === 0 && filter === 'all')
        return null;
    const totalDuration = subjTasks.reduce((acc, curr) => acc + curr.currentDuration + curr.history.reduce((hAcc, h) => hAcc + h.duration, 0), 0);
    const completedCount = subjTasks.filter((t) => t.status === 'completed').length;
    const progress = subjTasks.length > 0 ? Math.round((completedCount / subjTasks.length) * 100) : 0;
    const conf = SUBJECT_CONFIG[subject];
    const filteredTasks = subjTasks.filter((t) => filter === 'all' ? true : t.status === filter);
    ...
    return (<div className="relative">
       <div className="flex flex-col gap-2 mb-3">
          ...
          <div className="flex items-center justify-between px-1">
            <div className="flex gap-1 bg-white p-0.5 rounded-lg border border-slate-200">
                {['all', 'not_started', 'in_progress', 'completed'].map(f => (<button key={f} onClick={() => setFilter(f)} className={`px-2 py-1 text-[9px] font-bold rounded-md transition-all ${filter === f ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-50'}`}>
       ...
       <div className="space-y-4 pl-1 border-l-2 border-slate-100 ml-2">
         {Object.keys(tasksByCategory).length === 0 ? (<div className="text-center py-6 text-slate-400 text-[10px] font-bold">タスクがありません</div>) : (Object.keys(tasksByCategory).map(cat => (<div key={cat} className="pl-3">
                 <h4 className="text-[10px] font-bold text-slate-400 mb-2">{cat}</h4>
                 <div className="flex flex-col gap-2">
                   {tasksByCategory[cat].map((task) => (<TaskCard key={task.id} task={task} cycleStatus={cycleStatus} setDetailTaskId={setDetailTaskId}/>))}
                 </div>
              </div>)))}
       </div>
...
const DailyView = ({ tasks, updateLocalTask, syncTaskToCloud, cycleStatus, saveHistoryRecord, deleteUnitTasks, setAddModalOpen, selectedUnit, setSelectedUnit, unitsWithTasks, onAddCustomTask, setDetailTaskId, setDeleteConfirmation, pauseAllOtherTasks }) => {
    ...
    const allStats = useMemo(() => {
        ...
        const subjects = Object.keys(SUBJECT_CONFIG).map(subj => ({
            id: subj, ...getStats(tasks.filter((t) => t.subject === subj))
        }));
    ...
    return (<div className="flex flex-col h-full">
      <div className="sticky top-0 z-20 px-4 py-3 bg-slate-50 border-b border-slate-200">
        <div className="flex gap-2">
          ...
          <div className="flex-1 relative">
            <select value={selectedUnit || ''} onChange={(e) => e.target.value === 'NEW' ? setAddModalOpen(true) : setSelectedUnit(e.target.value)} className={`w-full h-full appearance-none rounded-xl font-bold text-xs pl-3 pr-8 focus:outline-none transition-all cursor-pointer ${selectedUnit ? 'bg-blue-600 text-white shadow-md shadow-blue-200' : 'bg-white border border-slate-200 text-slate-600'}`}>
      ...
      <div className="pt-4 px-4 space-y-6 pb-32">
        {selectedUnit ? (<div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
              {(() => {
                const unitTasks = tasks.filter((t) => t.unit === selectedUnit);
                const { progress, totalTime } = getStats(unitTasks);
                return (<div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 relative overflow-hidden">
              ...
              {Object.keys(SUBJECT_CONFIG).map(subj => (<SubjectSection key={subj} unit={selectedUnit} subject={subj} tasks={tasks} cycleStatus={cycleStatus} setDetailTaskId={setDetailTaskId} onAddCustomTask={onAddCustomTask}/>))}
          </div>) : (<div className="space-y-5 animate-in fade-in">
              ...
              {unitsWithTasks.map((unit) => {
                ...
                const { progress, totalTime } = getStats(unitTasks);
                const subjStats = Object.keys(SUBJECT_CONFIG).map(subj => ({ id: subj, ...getStats(unitTasks.filter((t) => t.subject === subj)) }));
                return (<div key={unit} onClick={() => setSelectedUnit(unit)} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 active:scale-[0.98] transition-all cursor-pointer relative">
                     <button onClick={(e) => {
                        e.stopPropagation();
                     ...
                     <div className="grid grid-cols-4 gap-2 pt-3 border-t border-slate-50">
                        {subjStats.map(s => {
                        const conf = SUBJECT_CONFIG[s.id];
                        return (<div key={s.id} className="flex flex-col gap-1">
...
const TestsView = ({ tests, onSaveTest, onDeleteTest }) => {
    ...
    const toggleSubject = (subj) => { setSelectedSubjects(prev => prev.includes(subj) ? prev.filter(s => s !== subj) : [...prev, subj]); };
    const chartData = useMemo(() => {
        let filtered = filterType !== 'all' ? tests.filter((t) => t.type === filterType) : [...tests];
        return filtered.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map((t) => {
            ...
            if (selectedSubjects.includes('4ko'))
                dp['4ko'] = t.total4.dev;
            ['math', 'japanese', 'science', 'social'].forEach(s => { if (selectedSubjects.includes(s))
    ...
    return (<div className="flex flex-col h-full bg-slate-50">
       <div className="bg-white sticky top-0 z-20 px-4 py-3 border-b border-slate-200 shadow-sm space-y-2.5">
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-0.5 no-scrollbar">
             ...
             {['math', 'japanese', 'science', 'social'].map(s => <button key={s} onClick={() => toggleSubject(s)} className={`flex-1 py-1.5 px-2 rounded-lg text-[10px] font-bold whitespace-nowrap transition-colors ${selectedSubjects.includes(s) ? `${SUBJECT_CONFIG[s].bg} text-white` : 'bg-slate-100 text-slate-500'}`}>{SUBJECT_CONFIG[s].short}</button>)}
          </div>
       </div>
       ...
       <div className="flex-1 overflow-y-auto px-3 py-4 space-y-5 pb-20">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <table className="w-full text-center text-[10px]">
               <thead className="text-slate-400 bg-slate-50 border-y border-slate-100"><tr><th className="py-2 pl-3 text-left font-bold">テスト名</th><th className="py-2 font-bold">4科</th>{['math', 'japanese', 'science', 'social'].map(s => <th key={s} className={`py-2 font-bold ${SUBJECT_CONFIG[s].color}`}>{SUBJECT_CONFIG[s].short}</th>)}<th className="py-2 w-12"></th></tr></thead>
               <tbody>
                  {tableData.map(t => (<tr key={t.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors group">
                        ...
                        {['math', 'japanese', 'science', 'social'].map(s => <td key={s} className={`py-2 font-bold ${t.subjects[s].dev >= 60 ? 'text-rose-500' : 'text-slate-500'}`}>{t.subjects[s].dev}</td>)}
...
const AchievementsView = ({ tasks }) => {
    ...
    const [selectedSubjects, setSelectedSubjects] = useState(['math', 'japanese', 'science', 'social']);
    const toggleSubject = (subj) => { setSelectedSubjects(prev => prev.includes(subj) ? prev.filter(s => s !== subj) : [...prev, subj]); };
    ...
    const { chartData, pieData, maxStats } = useMemo(() => {
        ...
        const totalBySubject = { math: 0, japanese: 0, science: 0, social: 0 };
        tasks.forEach(task => {
            task.history.forEach(h => {
                ...
                if (hDate >= start && hDate <= end) {
                    const key = `${m}/${d}`;
                    if (!dateMap.has(key))
                        dateMap.set(key, { name: key, math: 0, japanese: 0, science: 0, social: 0, total: 0 });
        ...
        const pData = Object.entries(totalBySubject).filter(([k, v]) => selectedSubjects.includes(k) && v > 0).map(([k, v]) => ({
            name: SUBJECT_CONFIG[k].label, value: v, color: SUBJECT_CONFIG[k].hex,
        ...
        for (const d of dateMap.values()) {
            if (d.total > maxTotal.val)
                maxTotal = { val: d.total, date: d.name };
            ['math', 'japanese', 'science', 'social'].forEach(s => { if (d[s] > maxSubj[s].val)
                maxSubj[s] = { val: d[s], date: d.name }; });
        }
    ...
    return (<div className="flex flex-col h-full bg-slate-50">
       <div className="bg-white sticky top-0 z-20 px-4 py-3 border-b border-slate-200 shadow-sm space-y-2.5">
          <div className="flex gap-2">
            ...
            { l: '2週間', d: 14 },
            { l: '1ヶ月', d: 30 }
        ].map(r => (<button key={r.l} onClick={() => setPresetRange(r.d)} className="flex-1 py-1.5 bg-white border border-slate-100 text-xs font-bold rounded-lg text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors shadow-sm">
                    {r.l}
                  </button>))}
             </div>
          </div>
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-0.5 no-scrollbar">
             {['math', 'japanese', 'science', 'social'].map(s => (<button key={s} onClick={() => toggleSubject(s)} className={`flex-1 py-1.5 px-2 rounded-lg text-[10px] font-bold whitespace-nowrap transition-colors ${selectedSubjects.includes(s) ? `${SUBJECT_CONFIG[s].bg} text-white` : 'bg-slate-100 text-slate-400'}`}>{SUBJECT_CONFIG[s].short}</button>))}
          </div>
       </div>
             ...
             <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 min-h-[12rem] flex flex-col">
                ...
                {pieData.length > 0 ? (<>
                    ...
                    <div className="flex flex-col gap-1">
                        {pieData.map(d => (<div key={d.name} className="flex justify-between items-center text-[9px]">
             ...
             <div className="space-y-3">
                ...
                <div className="bg-white rounded-2xl p-2.5 border border-slate-200 shadow-sm">
                   ...
                   <div className="space-y-1">
                      {['math', 'japanese', 'science', 'social'].map(s => (selectedSubjects.includes(s) && maxStats.subjects[s].val > 0 && (<div key={s} className="flex justify-between items-center text-[9px]">
...
// ==========================================
const LoginForm = ({ onLogin, onSampleMode }) => {
    ...
    return (<div className="flex h-screen items-center justify-center bg-slate-50 px-4">
...
export default function App() {
    ...
    const fetchData = useCallback(async (isSilent = false) => {
        ...
        if (!dbInstance || !auth?.currentUser || isSampleMode) {
            if (!dbInstance && !isSilent)
                console.warn('Firebase未設定のため、データ取得をスキップしサンプルモードで動作します');
            return;
        }
        if (!isSilent)
    ...
    const syncTaskToCloud = async (id, cloudUpdates) => {
        const dbInstance = getSafeDb();
        if (!dbInstance || !auth?.currentUser || isSampleMode)
            return;
        try {
            ...
            CURRICULUM_PRESETS[subject].forEach(preset => {
                preset.items.forEach((item, index) => {
                    newTasks.push({
                        ...
                        unit: unitName, subject: subject, category: preset.category, title: item,
    ...
    const onAddCustomTask = async (unit, subject, title, category) => {
        const newTask = {
    ...
    // --- Routing / Auth Gate ---
    if (isAuthChecking)
        return <div className="flex h-screen items-center justify-center bg-slate-50 text-slate-400 font-bold">起動中...</div>;
    if (!user && !isSampleMode) {
    ...
    return (<div className="flex flex-col h-screen bg-slate-50 font-sans text-slate-900 max-w-md mx-auto shadow-2xl overflow-hidden relative">
      {isSampleMode && (<div className="bg-amber-100 text-amber-800 text-[10px] font-bold text-center py-1 flex items-center justify-center gap-1.5 z-40 relative">
      ...
      <header className="bg-white/80 backdrop-blur-xl pt-4 sticky top-0 z-30 border-b border-slate-100">
        <div className="h-14 flex items-center justify-between px-5">
          ...
          <div className="flex items-center gap-2">
             {!isSampleMode ? (<div className="flex items-center gap-2">
               ...
               </div>) : (<button onClick={() => { setIsSampleMode(false); fetchData(true); }} className="text-[10px] bg-slate-800 text-white px-3 py-1.5 rounded font-bold transition-colors active:scale-95">
                  データに戻る
                </button>)}
             <button onClick={() => {
            if (!isSampleMode) {
        ...
        }} className={`text-[10px] px-2 py-2 rounded-lg font-bold transition-colors flex items-center gap-1 active:scale-95 ${isSampleMode ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`} title="サンプル表示">
      ...
      <main className="flex-1 overflow-y-auto overscroll-contain no-scrollbar flex flex-col pb-24">
        {activeTab === 'daily' ? (<DailyView tasks={tasks} updateLocalTask={updateLocalTask} syncTaskToCloud={syncTaskToCloud} cycleStatus={cycleStatus} saveHistoryRecord={saveHistoryRecord} deleteUnitTasks={deleteUnitTasks} deleteTask={(id) => {
                setConfirmModalData({
      ...
      {detailTaskId && (<TaskDetailModal task={tasks.find((t) => t.id === detailTaskId)} onClose={() => setDetailTaskId(null)} updateLocalTask={updateLocalTask} syncTaskToCloud={syncTaskToCloud} onSaveRecord={saveHistoryRecord} onDelete={() => {
                setConfirmModalData({
                    title: 'タスクの削除', message: '本当にこのタスクを削除しますか？',
                    onConfirm: async () => {
