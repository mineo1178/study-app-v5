...
import { canRecruitThirdMember, claimRewards, getSessionActivityPoints, getUnclaimedRewards, getYunaRecruitmentStatus, joinNextMember, lessonMember, recruitThirdMember, recruitYuna } from "./game/rewards";
...
import { NOVA_STAGE_1, SPARKLE_STAGES } from "./game/config";
import { calculateAudience, calculateFanGain, canPerformLive, createSong, getCurrentVenue, getLiveCost, getLiveRating, isRegionalHallSoldOut, isVenueSoldOut } from "./game/song-live";
import { applySongAffinityToBattleStats, calculateBattleStats, calculateRivalBattleResult, canStartNovaStage1, canStartRivalBattle, canStartSparkleStage2, getRivalBattleRewards } from "./game/rival-battle";
...
interface TestResult {
  ...
  subjects: Record<
    Subject,
    { score: number; avg: number; dev: number; rank?: string }
  >;
...
const CURRICULUM_PRESETS: Record<
  Subject,
...
const StrictTimer = React.memo(
  ({
    ...
    onSaveRecord: (task: Task) => Promise<void>;
    ...
    const reviewFlagsRef = useRef<SessionReviewFlag[]>(task.sessionReviewFlags || []);
    const isRunning = task.isRunning;

    // 正確な現在秒数を計算するヘルパー（バックグラウンド等で表示が遅延しても正しい秒数を得る）
    const getAccurateSeconds = useCallback(() => {
      return getElapsedSeconds(task);
    }, [task]);
    ...
    const handlePlay = async (e?: React.MouseEvent) => {
      e?.stopPropagation();
      if (task.isRunning || startInFlightRef.current || isSaving) return;
    ...
    const handleStopAndSave = async () => {
      if (finishInFlightRef.current || isSaving || localSeconds === 0) return;
      finishInFlightRef.current = true;
      setIsSaving(true);
      const finalDuration = getAccurateSeconds(); // 古いlocalSecondsではなく正確な時間を再計算
      const now = Date.now();
      const reviewFlags = getSessionReviewFlags(finalDuration, reviewFlagsRef.current);
    ...
    return (
      <div className="flex flex-col gap-3 md:gap-4 w-full relative">
        ...
        <button
          ...
          disabled={localSeconds === 0 || isSaving}
...
const TestResultModal = ({
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: TestResult) => void;
  initialData?: TestResult | null;
}) => {
      ...
      <div className="bg-white w-full max-w-sm md:max-w-xl lg:max-w-2xl rounded-3xl md:rounded-[2rem] p-5 md:p-8 shadow-2xl">
        ...
        <div className="space-y-3 md:space-y-5">
          <div className="grid grid-cols-2 gap-3 md:gap-5">
            <div>
              <label className="block text-[10px] md:text-xs lg:text-sm font-bold text-slate-400 mb-1 md:mb-2">
                実施日
              </label>
              <input
                type="date"
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-lg md:rounded-xl px-2 py-1.5 md:px-3 md:py-2 text-sm md:text-base font-bold focus:outline-none focus:border-blue-400"
              />
            </div>
            <div>
              <label className="block text-[10px] md:text-xs lg:text-sm font-bold text-slate-400 mb-1 md:mb-2">
          ...
          <div className="border-t border-slate-100 pt-3 md:pt-5">
            <h4 className="font-bold text-slate-600 mb-2 md:mb-3 text-xs md:text-sm">
              各教科の偏差値
            </h4>
            <div className="grid grid-cols-4 gap-2 md:gap-3">
              {(["math", "japanese", "science", "social"] as Subject[]).map(
                (subj) => (
                  <div key={subj}>
                    <label
                      className={`block text-[10px] md:text-xs lg:text-sm text-center font-bold ${SUBJECT_CONFIG[subj].color} mb-1 md:mb-2`}
                    >
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg md:rounded-xl px-1 py-1.5 md:px-2 md:py-2 text-xs md:text-sm text-center font-bold focus:outline-none focus:border-blue-400"
                    />
                  </div>
                ),
              )}
  ...
  return (
    <div className="relative">
      ...
      <AddCustomTaskModal
        ...
        unit={unit}
        subject={subject}
        onAdd={(title: string, category: string) =>
...
const TodayStudyTimeline = ({ tasks }: { tasks: Task[] }) => {
  ...
  const displaySessions = useMemo<TodayTimelineSession[]>(() => {
    ...
    const normalized: TodayTimelineSession[] = [];
    ...
    sessions.forEach((session) => {
      ...
      const nextSession: TodayTimelineSession = {
        ...session,
        startAt,
        endAt,
      ...
      const preserved: TodayTimelineSession[] = [];
  ...
  const groupedSessions = (Object.keys(SUBJECT_CONFIG) as Subject[]).map(
    (subject) => ({
      subject,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-5 lg:gap-6">
              {unitsWithTasks.map((unit: string) => {
                ...
                const subjStats = (
                  ...
                  id: subj,
                  ...getStats(
                    unitTasks.filter((t: Task) => t.subject === subj),
                  ),
                }));
...
const TestsView = ({ tests, onSaveTest, onDeleteTest }: any) => {
  ...
  const chartData = useMemo(() => {
    let filtered =
      filterType !== "all"
        ? tests.filter((t: TestResult) => t.type === filterType)
        : [...tests];
    return filtered
      .sort(
      ...
      .map((t: TestResult) => {
        ...
        ["math", "japanese", "science", "social"].forEach((s) => {
          if (selectedSubjects.includes(s))
            dp[s] = t.subjects[s as Subject].dev;
        });
        return dp;
      });
    ...
    <div className="flex flex-col h-full bg-slate-50">
      <div className="bg-white sticky top-0 z-20 px-4 py-3 md:px-6 md:py-4 lg:px-8 lg:py-4 border-b border-slate-200 shadow-sm space-y-2.5 md:space-y-3">
        <div className="flex justify-between items-center gap-2 md:gap-3">
          <div className="flex gap-1.5 md:gap-2">
            <button
              ...
              className={`px-2.5 py-1.5 md:px-3 md:py-1.5 rounded-lg text-[10px] md:text-[11px] lg:text-xs font-bold border ${filterType === "all" ? "bg-slate-800 text-white border-slate-800" : "bg-white border-slate-200 text-slate-500"}`}
            >
              全て
            </button>
            {Object.entries(TEST_TYPE_CONFIG).map(([k, v]) => (
              <button
                ...
                className={`px-2.5 py-1.5 md:px-3 md:py-1.5 rounded-lg text-[10px] md:text-[11px] lg:text-xs font-bold border ${filterType === k ? v.activeClass : "bg-white border-slate-200 text-slate-500"}`}
              >
                {v.label}
              </button>
            ))}
        ...
        <div className="flex gap-1.5 md:gap-2 overflow-x-auto pb-0.5 md:pb-1 no-scrollbar">
          <button
            ...
            className={`flex-1 py-1.5 px-2 md:py-1.5 md:px-3 rounded-lg text-[10px] md:text-[11px] lg:text-xs font-bold whitespace-nowrap transition-colors ${selectedSubjects.includes("4ko") ? "bg-slate-800 text-white" : "bg-slate-100 text-slate-500"}`}
          >
            4科
          </button>
          {(["math", "japanese", "science", "social"] as Subject[]).map((s) => (
            <button
              ...
              className={`flex-1 py-1.5 px-2 md:py-1.5 md:px-3 rounded-lg text-[10px] md:text-[11px] lg:text-xs font-bold whitespace-nowrap transition-colors ${selectedSubjects.includes(s) ? `${SUBJECT_CONFIG[s].bg} text-white` : "bg-slate-100 text-slate-500"}`}
            >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 md:px-6 md:py-6 lg:px-8 lg:py-8 space-y-5 md:space-y-6 lg:space-y-8 pb-20 md:pb-32">
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
          <h3 className="font-bold text-slate-700 text-xs md:text-sm lg:text-base px-3 pt-3 pb-2 md:px-5 md:pt-5 md:pb-3 lg:px-6 lg:pt-6 flex items-center gap-1.5 md:gap-2">
            <TrendingUp className="text-blue-500 w-[14px] h-[14px] md:w-4 md:h-4 lg:w-5 lg:h-5" />{" "}
            偏差値履歴
          </h3>
          <table className="w-full text-center text-[10px] md:text-xs lg:text-sm">
            <thead className="text-slate-400 bg-slate-50 border-y border-slate-100">
              <tr>
                <th className="py-2 md:py-3 lg:py-4 pl-3 md:pl-5 text-left font-bold">
                  テスト名
                </th>
                <th className="py-2 md:py-3 lg:py-4 font-bold">4科</th>
                {(["math", "japanese", "science", "social"] as Subject[]).map(
                  (s) => (
                    <th
                      key={s}
                      className={`py-2 md:py-3 lg:py-4 font-bold ${SUBJECT_CONFIG[s].color}`}
                    >
                      {SUBJECT_CONFIG[s].short}
                    </th>
                  ),
                )}
                <th className="py-2 w-12 md:w-16"></th>
              </tr>
            </thead>
            <tbody>
              {tableData.map((t) => (
                ...
                >
                  <td className="py-2 md:py-3 lg:py-4 pl-3 md:pl-5 text-left">
                    <div className="text-[8px] md:text-[9px] lg:text-[10px] text-slate-400 font-bold">
                      {t.date}
                    </div>
                    <div className="font-bold text-slate-700 truncate max-w-[80px] md:max-w-none">
                      {t.name}
                    </div>
                  </td>
                  <td className="py-2 md:py-3 lg:py-4 font-black text-slate-700 bg-slate-50/50">
                    {t.total4.dev}
                  </td>
                  {(["math", "japanese", "science", "social"] as Subject[]).map(
                    (s) => (
                      <td
                        key={s}
                        className={`py-2 md:py-3 lg:py-4 font-bold ${t.subjects[s].dev >= 60 ? "text-rose-500" : "text-slate-500"}`}
                      >
                        {t.subjects[s].dev}
                      </td>
                    ),
...
export default function App() {
  ...
  const runLesson = async (memberId: string) => {
    const focus = memberId === "science" ? "dance" : memberId === "japanese" ? "lyrics" : memberId === "social" || memberId === "yuna" ? "character" : "vocal";
  ...
  const recruitYunaMember = async () => {
    if (!getYunaRecruitmentStatus(game).canRecruit) return;
    if (isSampleMode || !auth?.currentUser || !getSafeDb()) {
      setGame(recruitYuna(game));
      return;
    }
    const dbInstance = getSafeDb()!;
    try {
      const next = await runTransaction(dbInstance, async (transaction) => {
        const ref = getGameDoc(dbInstance);
        const snapshot = await transaction.get(ref);
        const current = snapshot.exists() ? normalizeGameState(snapshot.data() as Partial<ProducerGameState>) : createInitialGameState();
        const updated = recruitYuna(current);
        if (updated === current) throw new Error("YUNA_NOT_AVAILABLE");
  ...
  const changeLeader = async (memberId: string) => {
    const next = setLeader(game, memberId);
    if (next === game) return;
    setGame(next);
    const dbInstance = getSafeDb();
    if (!isSampleMode && dbInstance && auth?.currentUser) {
  ...
  const performFirstLive = async (songId: string) => {
    const selectedSong = game.songs.find((item) => item.id === songId);
    if (!selectedSong || !canPerformLive(game, selectedSong)) return;
    const performanceId = `performance-${crypto.randomUUID?.() || Date.now()}`;
    const createResult = (current: ProducerGameState): { next: ProducerGameState; performance: Performance } => {
      const song = current.songs.find((item) => item.id === songId); const venue = getCurrentVenue(current, performances);
      if (!song) throw new Error("SONG_NOT_AVAILABLE");
      if (!canPerformLive(current, song, venue.id)) throw new Error("LIVE_NOT_AVAILABLE");
      const audience = calculateAudience(current, song, venue.id); const rating = getLiveRating(calculateAudience(current, song, venue.id));
      const leaderBonus = applyLeaderSkillToLiveFanGain(current, calculateFanGain(audience, rating, song));
      const performance: Performance = { performanceId, songId: song.id, venueId: venue.id, performedAt: Date.now(), audience, capacity: venue.capacity, rating, fanGain: leaderBonus.finalFanGain, fanBefore: current.fans, fanAfter: current.fans + leaderBonus.finalFanGain, version: APP_VERSION, formation: getFormationSnapshot(current), leaderBonus: leaderBonus.skillId ? leaderBonus : null };
      const next = { ...current, activityPoints: current.activityPoints - getLiveCost(venue.id), fans: current.fans + leaderBonus.finalFanGain, milestones: { ...current.milestones, miniLiveHouseSoldOut: current.milestones?.miniLiveHouseSoldOut || isVenueSoldOut(performance), regionalHallSoldOut: current.milestones?.regionalHallSoldOut || isRegionalHallSoldOut(performance), cityHallSoldOut: current.milestones?.cityHallSoldOut || (performance.venueId === "city-hall" && performance.audience === 800) }, songs: current.songs.map((item) => item.id === song.id ? { ...item, performanceCount: item.performanceCount + 1 } : item) };
      return { next, performance };
    };
    if (isSampleMode || !auth?.currentUser || !getSafeDb()) { const result = createResult(game); setGame(result.next); setPerformances((current) => [result.performance, ...current].slice(0, 5)); return; }
    const dbInstance = getSafeDb()!;
  ...
  const startSparkleBattle = async (songId: string, stage = 1) => {
    ...
    const createResult = (current: ProducerGameState): { next: ProducerGameState; record: RivalBattleRecord } => {
      const currentSong = current.songs.find((item) => item.id === songId); const allowed = isNova ? canStartNovaStage1(current, performances) : stage === 2 ? canStartSparkleStage2(current, performances) : canStartRivalBattle(current, performances); if (!currentSong || currentSong.status !== "completed" || !allowed) throw new Error("BATTLE_NOT_AVAILABLE");
      const basePlayer = calculateBattleStats(current, currentSong); const leaderPlayer = applyLeaderSkillToBattleStats(current, basePlayer); const player = stage === 1 ? leaderPlayer : applySongAffinityToBattleStats(leaderPlayer, currentSong); const outcome = calculateRivalBattleResult(player, stage, isNova ? NOVA_STAGE_1.stats : undefined); const alreadyClaimed = current.claimedRivalBattleIds?.includes(battleId) ?? false; const alreadyWon = current.wonRivalBattleIds?.includes(battleId) ?? false; const reward = getRivalBattleRewards(outcome.overallResult, alreadyWon, alreadyClaimed, stage, isNova ? "nova" : "sparkle");
      const record: RivalBattleRecord = { battleId, attemptId: `attempt-${crypto.randomUUID?.() || Date.now()}`, rivalId: isNova ? "nova" : "sparkle", stage: isNova ? 1 : stage, playedAt: Date.now(), songId, selectedSongId: songId, categoryResults: outcome.categoryResults, overallResult: outcome.overallResult, fanGain: reward.fans, bonusPoints: reward.points, version: APP_VERSION, formation: getFormationSnapshot(current), baseCategoryStats: basePlayer, finalCategoryStats: player, rivalStats: stageConfig.stats, categoryScores: Object.fromEntries(Object.entries(player).map(([category, score]) => [category, { playerScore: score, rivalScore: stageConfig.stats[category as keyof typeof player], leaderBonusApplied: score - basePlayer[category as keyof typeof basePlayer] }])) };
      const won = outcome.overallResult === "PERFECT WIN" || outcome.overallResult === "WIN";
      return { record, next: { ...current, fans: current.fans + reward.fans, activityPoints: current.activityPoints + reward.points, rivalEventsCompleted: current.rivalEventsCompleted + reward.event, claimedRivalBattleIds: alreadyClaimed ? current.claimedRivalBattleIds : [...(current.claimedRivalBattleIds ?? []), battleId], wonRivalBattleIds: won && !current.wonRivalBattleIds?.includes(battleId) ? [...(current.wonRivalBattleIds ?? []), battleId] : current.wonRivalBattleIds } };
    };
    if (isSampleMode || !auth?.currentUser || !getSafeDb()) { const result = createResult(game); setGame(result.next); setRivalBattles((current) => [result.record, ...current.filter((item) => item.battleId !== battleId)]); return; }
    const dbInstance = getSafeDb()!;
    try { const result = await runTransaction(dbInstance, async (transaction) => { const gameRef = getGameDoc(dbInstance); const snapshot = await transaction.get(gameRef); const current = snapshot.exists() ? normalizeGameState(snapshot.data() as Partial<ProducerGameState>) : createInitialGameState(); const prepared = createResult(current); transaction.set(getRivalBattleDoc(dbInstance, `${battleId}-${prepared.record.playedAt}`), prepared.record); transaction.set(gameRef, prepared.next); return prepared; }); setGame(result.next); setRivalBattles((current) => [result.record, ...current]); } catch { setSyncState("offline"); }
  };
